...
def write_caption(srt_location: str, input_video: str, output_dir: str):
    with open(srt_location, 'r') as f:
        srt = f.read()
    srt_list = srt.split('\n')
    sec_index = 1
    text_index = 2
    ...
    video = VideoFileClip(input_video).resize((480, 240))
    ...
    def text_clip(text, name):
        ...
        draw.text(((480 - text_width) / 2, 200), text, fill=(255, 255, 255), font=font, stroke_width=2,
                  stroke_fill='black')
        img.save(name)
    ...
    output.write_videofile(output_dir, temp_audiofile="temp-audio.m4a", remove_temp=True, codec="libx264",
                           audio_codec="aac")
